...
export function HostedHistoryToolbar({ group, room, capability, onResults }: {
  ...
  if (!canSearch && !canRead) {return null}
...
export function HostedThreadActions({ group, thread, throughSeq, capability }: {
  group: string; thread: string; throughSeq: number; capability: Capability
}) {
